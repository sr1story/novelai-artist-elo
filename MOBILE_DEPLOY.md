...
4. 다음 비교부터 임시 목록의 작가를 한 명씩 A/B로 평가합니다.
5. **임시 풀 종료**를 누르면 현재 비교는 유지하고, 그다음 비교부터 기존 활성 풀로 돌아갑니다.

임시 비교의 ELO와 기록은 저장되지만 기존 활성 풀의 작가를 추가하거나 제거하지 않습니다. 임시 목록과 활성 상태도 영구 디스크에 보존됩니다.
...
- 붙여넣은 작가 목록으로 만든 임시 탐색 풀과 활성 상태
- 최근 비교 상태
- 프롬프트·이미지 설정 프리셋 10개
- 생성 이미지와 CSV 내보내기
